...
var parser = {
  parse: function (script, _options, callback) {
    ...
    var tokens = tokenizer.tokenize(script, options.language),
        token,
        title_page_html = [],
        script_html = [];
  ...
  lexer: function (s) {
    ...
    s = s.replace(sections._common.note_inline, inline.note).replace(/\\\*/g, '[star]').replace(/\\_/g, '[underline]').replace(/\n/g, inline.line_break);

    for (var i in styles) {
      style = styles[i];
      match = sections['_common'][style];
